...
('Para tachões, perguntar OBRIGATORIAMENTE se o cliente deseja Monodirecional ou Bidirecional. Se o cliente for leigo ou perguntar, explicar que Monodirecional reflete em apenas um sentido e Bidirecional reflete nos dois sentidos (dependendo se a via tem fluxo em um ou dois sentidos).'),
('Para o cálculo de tachões (tartarugas) usados como redutores de velocidade, considere sempre 4 unidades por metro linear (25cm cada), sem espaçamento entre eles (Ex: 50m = 200 tachões), a menos que o cliente solicite algo diferente.')
ON CONFLICT DO NOTHING;
